...
class _DocumentVerificationScreenState extends State<DocumentVerificationScreen> {
  ...
  // State for holding the different image files
  ...
  Future<void> _startScan() async {
    ...
    final isMultiSide = provider.flowState.isDocumentVerificationMultiSide;
    ...
    // Add a small delay to help prevent the "Reply already submitted" crash on back press.
  ...
  Future<void> _submitManual() async {
    if (_apiFrontImage == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please upload the front side image first.')));
      return;
    }
    final provider = context.read<Verification>();
    final success = await provider.submitDocument(context, front: _apiFrontImage!, back: _apiBackImage);
    if (!mounted) return;
    if (success) {
  ...
  Future<void> _submitScanned(File front, File? back) async {
    final provider = context.read<Verification>();
    final success = await provider.submitDocument(context, front: front, back: back);
    if (!mounted) return;
    if (success) {
      provider.nextScreen(context);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(provider.errorMessage ?? 'Scan submission failed. Please try again.'),
        ...
        return Dialog(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Padding(
              ...
              child: Transform.rotate(angle: rotationAngle * (math.pi / 180), child: Image.file(image)),
            ),
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<Verification>();
    final isLoading = provider.isLoading;
    final isManualScan = provider.flowState.isDocumentVerificationManualScan;
    final isMultiSide = provider.flowState.isDocumentVerificationMultiSide;

    final buttonStyle = TextButton.styleFrom(
      backgroundColor: provider.designSettings?.settings?.secondaryColor,
      foregroundColor: provider.designSettings?.settings?.buttonTextColor,
...
/// A service that encapsulates all the BlinkID scanning logic.
class DocumentScannerService {
  bool _isScanInProgress = false; // Flag to prevent concurrent scans
  ...
  Future<ScanResult?> scanMultiSideDocument() async {
    if (_isScanInProgress) return null;
    ...
    try {
      ...
      for (var result in results) {
        if (result is BlinkIdMultiSideRecognizerResult) {
          ...
          File? displayBack, apiBack;
          if (result.fullDocumentBackImage?.isNotEmpty == true && result.backCameraFrame?.isNotEmpty == true) {
